---
name: 코덱스플로우
description: "Use when the user types $코덱스플로우, $코덱스플로우 상태, 라우트, 다음실행, 모두실행, 브리핑, 리뷰, PR초안, or 병합. 라우트는 요청을 ticket/plan queue로 만들고, 다음실행은 다음 ready unit을 run-next로 실행하며, 모두실행은 ready unit들을 run-all로 자동 실행한다."
metadata:
...
| `$코덱스플로우 라우트 <요청>` | 라우트 | 요청을 ticket으로 저장하고 plan queue를 만든다. PR lock이 있으면 `--auto-resolve`로 stacked plan을 만든다. |
