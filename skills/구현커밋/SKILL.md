---
name: 구현커밋
description: "Use when the user wants the canonical 구현커밋 workflow orchestrator for plan-first implementation, isolated worktree execution, commit-unit execution, review, repair, PR dry-run, PR creation, merge readiness, or morning briefs."
metadata:
...
구현커밋는 큰 작업을 `ticket -> plan/branch/worktree -> run-next -> Codex exec -> review decision -> commit -> run-all -> AI preflight repair -> PR/merge -> morning brief`로 나누는 자동 개발 공장 스킬이다.
...
중요한 기본값: hard stop은 사용자에게 되묻는 장치가 아니라 AI preflight가 먼저 해결해야 하는 작업 신호다. 구현커밋을 쓸 때는 가능한 경우 `--auto-resolve` 경로로 dirty state, unfinished unit, merge readiness를 먼저 정리하고 계속 진행한다. 단 PR lock은 review gate라서 새 작업을 inbox로 보낸다.
...
## Worktree And Artifact Lane Discipline

구현커밋의 기본 안전 단위는 branch가 아니라 `lane-specific worktree + branch`다. 장기 작업 브랜치 하나에 memory, `.codex-flow`, docs, code/test를 계속 쌓으면 main 병합 의미가 깨진다.

실행 전 항상 변경 표면을 아래 lane 중 하나 이상으로 분류한다.

- `code-test`: `src/`, `scripts/`, `tests/`, 앱 코드, 런타임 코드
- `docs-research`: `docs/`, research, visual/explainer artifact
- `codex-flow`: `.codex-flow/`, queue, ticket, plan runtime state
- `memory-sync`: `memory/`, journal, sessions, summaries, interpretations, ontology
- `mixed`: 위 lane이 둘 이상 섞인 상태

기본 규칙:

- 새 구현 unit은 현재 루트 작업공간에서 바로 실행하지 말고, `main` 또는 명시 source branch 기준의 isolated worktree에서 시작한다.
- worktree 이름과 branch 이름은 lane을 드러내야 한다. 예: `codex/<slug>-code`, `codex/<slug>-docs`, `codex/<slug>-flow`, `codex/<slug>-memory`.
- `mixed` 상태가 보이면 같은 branch에 계속 커밋하지 않는다. 먼저 lane별 split worktree를 만들고 path 단위로 회수한 뒤 branch별로 review/merge한다.
- `.codex-flow/`는 실행 queue와 운영 상태 lane이다. 코드 변경 branch에 무심코 같이 커밋하지 않는다. 단, 해당 구현커밋 plan 자체의 merge/review branch라면 `.codex-flow/` 전용 branch로 닫는다.
- `memory/`는 자동 capture/materialize lane이다. 코드 unit 커밋에 섞지 말고 `memory-sync` branch에서 별도 검증 후 병합한다.
- `docs/` research와 visual artifact는 `docs-research` branch에서 닫는다. 코드 변경과 같은 commit으로 묶는 것은 plan이 명시할 때만 허용한다.
- 병합은 clean `main` integration worktree에서 `--no-ff`로 branch별로 수행한다. 병합 뒤 불필요한 branch는 `git branch -d`가 성공할 때만 삭제한다. `-D`가 필요한 branch는 아직 미검토/미병합으로 보류한다.
...
1. 현재 repo의 status를 읽고 lane을 분류한다. dirty root에 mixed lane이 있으면 root에서 실행하지 말고 lane별 isolated worktree를 만든다.
2. 저장소 루트 또는 해당 isolated worktree에서 `/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py init`으로 `.codex-flow/`를 만든다.
3. `/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py route <plan-first.md>`로 승인된 plan-first Markdown 문서를 실행 source로 채택하고 plan/branch queue를 만든다. 짧은 자연어 요청으로 새 plan을 만들지 않는다.
4. `/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py run-next --plan <plan.md>`로 commit unit 하나를 구현하고 같은 Codex session에서 `review-all-in-one` post-unit gate를 통과한 뒤 자동 커밋한다. 프롬프트만 만들 때는 `--preview`, 큐 변경도 없이 볼 때는 `--dry-run`을 붙인다.
5. `/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py run-all --plan <plan.md>`로 plan이 complete 또는 needs_work가 될 때까지 반복 처리하며 각 성공 unit을 `review-all-in-one` post-unit gate 후 자동 커밋한다. `--auto-resolve`에서는 transient needs_work를 기본 1회 repair하고, preview prompt만 만들 때는 `--preview`를 붙인다.
6. 긴 Codex child process는 `--codex-timeout-seconds`로 제한한다. 기본값은 900초다. timeout 또는 nonzero child failure가 나면 해당 unit은 `needs_work`가 되고, `diagnostic_path`에 prompt/args/stdout/stderr/last-message/metadata 파일 위치가 남는다.
7. 아침에는 `morning-brief`와 `review`로 검토 자료를 만든다.
8. PR 초안은 `open-pr --auto-resolve --dry-run`으로 남은 unit을 먼저 끝내고 자동 커밋한 뒤 만든다. 실제 원격 PR은 `create-pr --auto-resolve`로 분리해 실행한다.
9. merge는 clean integration worktree에서 branch별로 진행한다. 미완료 unit이 있으면 `merge --auto-resolve`로 먼저 끝내고 자동 커밋한 뒤 readiness를 확인한다. 현재 작업 흐름상 원격 통합이 자연스러운 완료 조건이면 `--remote`까지 사용할 수 있다.
...
| `$구현커밋 모두실행` | 모두실행 | 현재 plan의 incomplete commit unit들을 `run-all --auto-resolve`로 끝까지 실행하고 unit별 자동 커밋한다. |
...
- 작업 시작 전에 lane을 분류한다. `memory/`, `.codex-flow/`, `docs/`, `scripts/tests/src`가 섞여 있으면 같은 commit이나 long-lived branch에 계속 쌓지 않는다.
- 구현 unit을 기존 dirty root worktree에서 바로 실행하지 않는다. clean isolated worktree를 만들거나 이미 해당 lane용 worktree가 있을 때만 이어간다.
...
- 실행 전 worktree가 dirty이면 먼저 lane을 분류한다. 같은 lane의 일시 dirty state만 `--auto-resolve` stash 대상으로 삼고, mixed lane이면 stash로 밀어붙이지 말고 split worktree recovery를 먼저 수행한다. revert/reset으로 사용자 변경을 삭제하지 않는다.
...
- `.codex-flow/`에는 로컬 작업 맥락이 들어갈 수 있으므로 공개 저장소에 올리기 전에 내용을 점검한다. 점검이 필요한 `.codex-flow/` 변경은 `codex-flow` lane branch로 분리한다.

## Auto-Resolve Policy

| 기존 hard stop | 구현커밋 자동 해결 |
| --- | --- |
| 작업 폴더가 더러우면 실행 안 함 | `run-next/run-all --auto-resolve`가 dirty path를 lane별로 분류한다. 같은 lane이면 stash로 보존 후 계속하고, mixed lane이면 split worktree recovery를 먼저 수행한다. |
...
git worktree add ~/.config/superpowers/worktrees/<repo>/<slug>-code -b codex/<slug>-code main
