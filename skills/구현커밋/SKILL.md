---
name: 구현커밋
description: "Use when the user wants the canonical 구현커밋 workflow orchestrator for plan-first implementation, task worktree execution, commit-unit execution, review, repair, safe finalize cleanup, PR dry-run, PR creation, merge readiness, or morning briefs."
metadata:
  short-description: "구현커밋 task-worktree implementation and finalize cleanup orchestrator"
---
...
구현커밋는 큰 작업을 `ticket/source plan -> task worktree -> task-local .codex-flow plan/queue -> run-next -> Codex exec -> review decision -> commit -> run-all -> AI preflight repair -> safe finalize cleanup -> morning brief`로 나누는 자동 개발 공장 스킬이다.
...
중요한 기본값: hard stop은 사용자에게 되묻는 장치가 아니라 AI preflight가 먼저 해결해야 하는 작업 신호다. 구현커밋을 쓸 때는 가능한 경우 `--auto-resolve` 경로로 dirty state, unfinished unit, merge readiness를 먼저 정리하고 계속 진행한다. 플랜의 모든 unit이 완료되면 기본값은 task branch를 target branch, 기본 `main`, 에 local merge하고, task-local `.codex-flow`를 archive한 뒤 생성된 task worktree를 non-force로 제거하고 `git branch -d`로 닫는 것이다. 단 PR lock은 review gate라서 새 작업을 inbox로 보낸다.

중요한 입력 기본값: 사용자가 계획 MD 경로를 매번 넘기게 하지 않는다. `$구현커밋`, `구현커밋 진행`, `플랜대로 실행`처럼 짧게 호출하면 구현커밋이 직전 대화, plan-first 최종 답변 링크, 현재 repo의 active `.codex-flow` plan, 최근 plan-first Markdown 산출물을 순서대로 확인해 단일 source plan을 자동 탐지한다. 후보가 없거나 둘 이상이면 그때만 후보와 이유를 보여주고 Operator에게 고르게 한다.
...
- `route <plan-first.md>`는 git repo에서 source repo를 오래 쓰지 않고 task worktree를 생성하거나 재사용한 뒤, 그 worktree 안의 task-local `.codex-flow/`에 tickets, plan, queue, prompts, attempts, logs를 만든다.
- `run-next`, `run-all`, PR, merge, source drift, mark는 plan metadata의 `execution_repo`/`source_repo` resolver를 사용한다. legacy plan은 기존 parent fallback을 유지한다.
...
1. 승인된 plan-first Markdown 문서를 자동 탐지하거나, 명시 경로/링크가 있으면 그것을 source로 채택한다.
2. 단일 source가 확정되면 `/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py route <plan-first.md>`가 task worktree를 만들거나 재사용하고, 그 안의 task-local `.codex-flow/`에 plan/queue를 만든다. 짧은 자연어 요청으로 새 plan을 만들지 않는다.
...
4. `/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py run-all --plan <task-worktree>/.codex-flow/plans/<slug>/plan.md`로 plan이 complete 또는 needs_work가 될 때까지 반복 처리하며 각 성공 unit을 `review-all-in-one` post-unit gate 후 자동 커밋한다. 모든 unit이 완료되면 별도 `--merge` 없이 target branch, 기본 `main`, 로 local merge하고 task-local `.codex-flow`를 archive한 뒤 생성된 task worktree를 제거하고 성공한 작업 브랜치를 `git branch -d`로 닫는다. `--auto-resolve`에서는 transient needs_work를 기본 1회 repair하고, preview prompt만 만들 때는 `--preview`, 완료 후 브랜치/worktree를 일부러 남길 때는 `--no-merge` 또는 `--keep-worktree`를 붙인다.
...
8. 별도 merge 명령은 중단된 finalize를 재개하거나 remote 통합이 필요한 경우에만 쓴다. 평상시 `$구현커밋 모두실행`과 `run-all --auto-resolve`는 완료 후 local merge, archive, worktree cleanup, branch close까지 진행한다. 현재 작업 흐름상 원격 통합이 자연스러운 완료 조건이면 `--remote`까지 사용할 수 있다.
...
## Plan Source Auto-Discovery

구현커밋은 계획 경로를 명시적으로 받지 않아도 먼저 source plan을 찾아야 한다.

탐지 우선순위:

1. 현재 사용자 입력의 Markdown 경로 또는 Markdown 링크
2. 직전 assistant 최종 답변의 `계획 MD:`, `검토 링크:`, `후행 실행:` 주변에 나온 plan-first Markdown 링크
3. 현재 repo의 active `.codex-flow` plan 또는 queue가 가리키는 source Markdown
4. 현재 대화에서 방금 만들거나 수정한 plan-first Markdown 파일
5. 현재 repo 안의 최근 plan-first 후보. 단, 넓은 전체 스캔 대신 `docs/plans/`, `docs/research/`, 명시된 작업 디렉터리처럼 좁은 후보군부터 확인한다.

단일 후보가 확정되면 Operator에게 경로를 다시 묻지 않고 route/run을 진행한다. 후보가 없거나 둘 이상이면 실행하지 말고, 후보 목록과 선택 이유를 짧게 보여준 뒤 어떤 계획을 실행할지 묻는다.

자동 탐지로 채택할 수 있는 source Markdown은 아래 조건 중 둘 이상을 만족해야 한다.

- `## Implementation Plan`이 있다.
- `### Commit N:` 또는 `### Phase N:` heading이 있다.
- `## Skill Routing Manifest`가 있다.
- `plan-first-implementation` 산출물임을 알 수 있는 `검토용 결과물`, `Plan Quality Check`, `구현 후 검토 리스트` 섹션이 있다.
- 직전 대화에서 사용자가 승인한 계획 문서로 언급됐다.

자동 탐지는 편의 기능이지 추측 실행 허가가 아니다. 단일성이 약하면 멈추고 묻는다.

## Shortcut Commands

| 호출 | 역할 | 실행 의미 |
| --- | --- | --- |
| `$구현커밋 <문서경로>` | 라우트 | 명시된 plan-first Markdown 문서 경로를 `route <문서경로> --auto-resolve`로 실행한다. |
| `$구현커밋 플랜대로 실행` | 라우트 | 현재 요청, 직전 대화 맥락, active plan context에서 단일 plan-first 문서를 자동 탐지해 route한다. 없거나 여러 개면 후보를 보여주고 묻는다. |
| `$구현커밋` | 자동 라우트 | 단일 plan-first source를 자동 탐지해 route한다. 이미 active plan이 있으면 다음 incomplete unit 실행 의도인지 확인하고, source가 없거나 여러 개면 후보를 보여주고 묻는다. |
...
| `$구현커밋 모두실행` | 모두실행 | 현재 plan의 incomplete commit unit들을 `run-all --auto-resolve`로 끝까지 실행하고 unit별 자동 커밋한 뒤, 완료되면 target branch로 local merge하고 `.codex-flow`를 archive하고 생성된 task worktree와 작업 브랜치를 닫는다. |
...
- `$구현커밋 <입력>`은 plan-first source route 또는 active plan execution 요청이다. `<입력>`이 `상태`, `다음실행`, `모두실행`, `브리핑`, `리뷰`, `PR초안`, `PR생성`, `병합`, `대시보드`, `PR체크` 같은 명시 명령이 아니면, 현재 입력, 직전 대화 맥락, active plan context에서 plan-first Markdown source를 자동 탐지한다.
- 현재 입력에 plan-first Markdown 경로/링크가 있으면 그것을 우선해 `route <문서경로> --auto-resolve`로 실행한다.
- 현재 입력이 `$구현커밋`, `플랜대로 실행`, `진행`, `이 계획대로`처럼 짧아도, 단일 plan-first source를 찾을 수 있으면 그 문서로 route한다.
- plan-first 문서를 찾을 수 없거나 후보가 여러 개면 실행하지 않는다. "경로를 넘겨 달라"만 말하지 말고, 확인한 후보와 모호한 이유를 짧게 보여준 뒤 어느 계획을 실행할지 묻는다.
- `$구현커밋`만 있고 요청문이 없어도 상태 확인으로 바꾸지 않는다. 먼저 source/active plan 자동 탐지를 시도하고, 단일 후보가 없을 때만 선택을 요청한다. 상태 확인은 `$구현커밋 상태`로만 실행한다.
...
- 완료된 `run-all`은 기본값으로 local merge, task-local `.codex-flow` archive, generated task worktree cleanup, branch close까지 수행한다. 브랜치/worktree를 남겨야 할 때만 `--no-merge` 또는 `--keep-worktree`를 명시한다.
- 완료된 task worktree cleanup은 force를 쓰지 않는다. `git worktree remove -f`와 `git branch -D`는 기본 경로에서 금지한다.
- 생성된 task worktree는 non-`.codex-flow` dirty path가 없고 branch가 target에 포함되며 `.codex-flow` archive가 확인된 뒤에만 제거한다.
- cleanup이 실패하면 `cleanup_held`로 보고하고 가능한 한 worktree/branch를 보존한다.
...
| 완료된 generated worktree가 남음 | 기본 `run-all --auto-resolve`가 task-local `.codex-flow`를 archive하고 `git worktree remove`로 non-force 제거한다. |
| `.codex-flow` diagnostics가 삭제될 위험 | archive verification 전에는 task-local `.codex-flow`를 제거하지 않는다. 실패하면 `cleanup_held`로 멈춘다. |
| cleanup이 부분 실패 | force 삭제 대신 archive 경로와 실패 이유를 남기고 재시도 가능한 상태로 보존한다. |
...
/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py route docs/plans/example-plan.md --worktree-root ~/.config/superpowers/worktrees/<repo>
...
/Users/moonsoo/projects/codex-flow/scripts/codex_flow.py run-all --plan .codex-flow/plans/<slug>/plan.md --auto-resolve --keep-worktree
