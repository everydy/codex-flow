...
def build_planner_prompt(input_data: PlannerAgentInput, plan_content: str = "") -> str:
    ...
    return "\n".join(
        [
            ...
            "- Include and maintain a `## Skill Routing Manifest` table before `## Commit Units`.",
            "- The manifest table columns must be `Phase | Required skills | Optional skills | Evidence`.",
            "- For every commit unit, list exact existing skill names a future Codex session should activate before implementation.",
            "- Include `plan-first-implementation` as a required skill for feature, UI/design/layout, refactor, integration, API/DB/routing, or other code implementation units.",
            "- Do not add `plan-first-implementation` to status, review, briefing, QA-only, or test-only units unless they also require an implementation plan gate.",
            "- If an implementation-looking unit intentionally omits `plan-first-implementation`, state the reason in the Evidence cell.",
            "- Use the manifest to route specialist work; do not paste skill bodies or duplicate their full instructions in the plan.",
            "- Keep `review-all-in-one` and `qa-gate` in `Final Gate` when the plan changes code, prepares PR/merge, or needs release readiness.",
            "",
